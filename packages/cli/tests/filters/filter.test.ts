...
describe('new Filter', () => {
  it('should return true', async () => {
    ...
    await expect(filter.test(new Workspace('wks', { name: 'wks' }, prj)))
      .resolves.toBe(true);
  });
...
describe('Filter.privateWorkspace', () => {
  it('should test for private workspace', async () => {
    ...
    await expect(filter.test(new Workspace('wks', { name: 'wks', private: true }, prj)))
      .resolves.toBe(true);

    await expect(filter.test(new Workspace('wks', { name: 'wks', private: false }, prj)))
      .resolves.toBe(false);
  });
...
describe('Filter.scripts', () => {
  it('should test for workspace with start script', async () => {
    ...
    await expect(filter.test(new Workspace('wks', { name: 'wks', scripts: { start: '' } }, prj)))
      .resolves.toBe(true);

    await expect(filter.test(new Workspace('wks', { name: 'wks', scripts: { test: '' } }, prj)))
      .resolves.toBe(true);
  });
});