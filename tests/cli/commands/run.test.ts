...
describe('jill run', () => {
  it('should run script in loaded workspace', async () => {
    ...
    await pipe$(yargs(), loggerMiddleware, planCommand(run, tasks$))
      .parseAsync('run test');
  ...
  it('should use given dependency selection mode', async () => {
    await pipe$(yargs(), loggerMiddleware, planCommand(run, var$()))
  ...
  it('should fail if script does not exists', async () => {
    ...
    await expect(
      pipe$(yargs(), loggerMiddleware, planCommand(run, var$()))
        .exitProcess(false)
